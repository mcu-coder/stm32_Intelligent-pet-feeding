...
//清空对应位置
void OLED_ShowClear(u8 x,u8 y,u8 n)
{      			    
	u8 t;
	for(;n!=0;n--)
	{
...
//显示一个整形变量的值
void OLED_ShowInt(u8 x,u8 y,int n)
{
	u8 i=0,chr[10]={'\0'};
	chr[0]='\0';
	chr[1]='0';	
...
//	RCC->APB2ENR|=(1<<3|1<<4);						//使能C,D,G组IO的时钟
//	GPIOB->CRL&=~0xf0000000;							//初始化对应IO
//	GPIOB->CRL|=0x30000000;									//设置工作方式为50mhz输出
//	GPIOB->CRH&=~0xf;							//初始化对应IO
//	GPIOB->CRH|=0x3;									//设置工作方式为50mhz输出
//	GPIOB->ODR|=1<<7|1<<8;
//	GPIOC->CRH&=~0xfff;							//初始化对应IO
//	GPIOC->CRH|=0x333;									//设置工作方式为50mhz输出
...
//GPIOC->ODR|=1<<13|1<<14|1<<15;
 ...
 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
 GPIO_Init(GPIOB, &GPIO_InitStructure);					 //根据设定参数初始化GPIOA.8
 GPIO_SetBits(GPIOB,GPIO_Pin_7);						 //PA.8 输出高
 ...
 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
 GPIO_Init(GPIOC, &GPIO_InitStructure);					 //根据设定参数初始化GPIOA.8
 GPIO_SetBits(GPIOC,GPIO_Pin_14);						 //PA.8 输出高

 GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;				 //LED0-->PA.8 端口配置
 GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
 GPIO_Init(GPIOC, &GPIO_InitStructure);					 //根据设定参数初始化GPIOA.8
 GPIO_SetBits(GPIOC,GPIO_Pin_15);						 //PA.8 输出高
