...
{ 
  ...
  //GPIO_PinRemapConfig(GPIO_FullRemap_TIM2,ENABLE);//Timer3部分重映射 TIM3_CH2→PB5
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_7| GPIO_Pin_10| GPIO_Pin_9; //TIM_CH1
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;  //复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  ...
  //TIM_DeInit(TIM3);
	...
	TIM_TimeBaseStructure.TIM_Prescaler =7099-1; //分频系数，72M/7200=10KHz,其他依此类推
	...
	//TIM_OCInitStructure.TIM_Pulse = 0; //设置待装入捕获比较寄存器的脉冲值
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High; //输出极性:TIM输出比较极性高(有效电平为高电平)
	TIM_OC1Init(TIM4, &TIM_OCInitStructure);  //根据TIM_OCInitStruct中指定的参数初始化外设TIMx
	TIM_OC1PreloadConfig(TIM4, TIM_OCPreload_Enable);  //CH2预装载使能	 
